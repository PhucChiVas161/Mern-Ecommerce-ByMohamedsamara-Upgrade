...
router.post("/add", async (req, res) => {
  try {
    ...
    await gmail.sendEmail(email, "merchant-application");
...
router.put("/:id/active", auth, async (req, res) => {
  try {
    ...
    if (!update.isActive) {
      ...
      await gmail.sendEmail(merchantDoc.email, "merchant-deactivate-account");
    }
...
const createMerchantUser = async (email, name, merchant, host) => {
  ...
  if (existingUser) {
    ...
    await gmail.sendEmail(email, "merchant-welcome", null, name);
    ...
    await gmail.sendEmail(email, "merchant-signup", host, {
      resetToken,
      email,
    });
